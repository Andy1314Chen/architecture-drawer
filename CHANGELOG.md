...
### Added
- **Evaluator cheatsheet + anti-simulation strategy** — `references/checks_cheatsheet.md`: all 16 quality checks in one table (exact default threshold, penalty formula with caps, trigger, repair) plus a report-line → repair decode map, so check semantics no longer require reading `evaluator.py`'s source. SKILL.md's fast path and the agent-replay prompts now state the strategy explicitly: the evaluator is a sub-second oracle — sketch an approximate layout, run `gen.py`, read the report lines, fix what they name; never mentally re-verify the layout against all checks. Motivated by session-trace attribution: 99.5% of replay wall-clock is LLM generation, 84% in two turns where the model reads evaluator source in ~15 windows and then simulates the evaluator in-head (~17 min replacing a 0.8s tool call).

- **Deterministic same-port spread (`connect()`)** — when ≥2 top-level `connect()` edges share one node side, endpoints now fan out symmetrically along the border (counterpart-order, ≤14px steps, skipped on sides shorter than 32px) instead of stacking into a single line that trips the duplicate-edge check. Offsets live on `Edge._spread_offsets`, fold into `_edge_rebuild`, and survive `relocate_node()` rerouting. Adapted from archify's `automaticPortSpread`. All 8 eval scores unchanged; only `mlir_pipeline` geometry changed (two fully-overlapped edges now ±14px apart) — golden regenerated.

- **Evidence-driven `auto_refine` via structured `Issue`s** — new `evaluator.Issue` (a `str` subclass rendering byte-identically to legacy report lines) carries `{code, subject, evidence}`; `check_spacing` and `check_composition`'s gutter rule emit them with stable codes (`spacing/too-close`, `composition/gutter`) and measured evidence (gap, axis, container rect). `auto_refine` dispatches on code with fix magnitudes computed from the measurement (exact centering / exact deficit +1px), replacing all report-string regex parsing and the fixed +20px heuristic. Partly adapted from archify's structured-diagnostics pattern.

- **SKILL.md bounded fast path** — a compact 3-step loop (write candidate → evaluate → ≤2 focused repair rounds, then truthful reporting) now leads the skill doc so agents start drawing before absorbing the full 16-check specification; the detailed sections remain as on-demand reference below it.
...
- **Replay semantic gate ran without the brief** — `_semantic_gate` in `tests/test_agent_replay.py` passed only `spec_text`, so the A/B/C design-brief checks never executed in the replay harness itself (an agent could write a coherently-wrong brief) and every refine round received a bogus `brief-absent` WARN for a brief the agent had already written. The gate now loads the sandbox's own `brief.json` (unparseable → treated as absent; `_check_artifacts` reports the parse failure) — brief-contract FAILs gate the run, and the WARN only appears when the brief is genuinely missing. Deterministic unit test: `test_semantic_gate_enforces_brief_contract`. `_check_artifacts` reuses the module-top `DesignBrief` import instead of re-inserting `SCRIPTS` into `sys.path` on every call.
