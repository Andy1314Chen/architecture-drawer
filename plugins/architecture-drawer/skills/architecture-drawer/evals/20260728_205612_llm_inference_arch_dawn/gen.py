"""LLM Distributed Inference Serving — 7-layer architecture diagram (Dawn palette).

Same 7-layer layout + spine as the blue/teal/amber/purple version, recolored
to a modern "dawn" palette. Four functional hue families, still 8 accents:
  indigo = control plane  (L1 Gateway, L2 Scheduler)
  mint   = compute        (L3 Inference Engine, L4 Parallelism)
  coral  = infra          (L5 Storage & Interconnect)
  lilac  = optimization   (L6 Hidden Optimizers, L7 Disaggregated)
Op cards stay neutral white + gray stroke so they never inflate the palette.
Type scale fixed at 4 tiers: 20 / 14 / 12 / 10.
"""
...
NAME = "llm_inference_arch_dawn"

# ---- palette (Dawn) -------------------------------------------------------
INDIGO_F, INDIGO_S = "#D9E2FF", "#4263EB"   # control plane
MINT_F,   MINT_S   = "#C3F0D8", "#12B886"   # compute
CORAL_F,  CORAL_S  = "#FFD8C2", "#E8590C"   # infra
LILAC_F,  LILAC_S  = "#EBD9F7", "#9C36B5"   # optimization
...
BANDS = [
    (1, "\u2460 \u63a5\u5165\u4e0e\u7f51\u5173\u5c42  Gateway Governance",            INDIGO_F, INDIGO_S, 138, 96),
    (2, "\u2461 \u5168\u5c40\u8c03\u5ea6\u4e0e\u8def\u7531  Scheduler & Routing",     INDIGO_F, INDIGO_S, 256, 120),
    (3, "\u2462 \u5206\u5e03\u5f0f\u63a8\u7406\u5f15\u64ce  Inference Engine",         MINT_F,   MINT_S,   400, 210),
    (4, "\u2463 \u6a21\u578b\u5e76\u884c\u4e0e\u52a0\u901f  Model Parallelism",        MINT_F,   MINT_S,   634, 150),
    (5, "\u2464 \u5f02\u6784\u5b58\u50a8\u4e0e\u901a\u4fe1  Storage & Interconnect",   CORAL_F,  CORAL_S,  808, 160),
    (6, "\u2465 \u9690\u6027\u4f18\u5316\u7ec4\u4ef6  Hidden Optimizers",             LILAC_F,  LILAC_S,  992, 130),
    (7, "\u2466 \u5206\u79bb\u5f0f\u90e8\u7f72  Disaggregated Serving",               LILAC_F,  LILAC_S,  1146, 150),
]
...
drawer.database(840, 294, 260, 58, fill=INDIGO_F, stroke=INDIGO_S, stroke_width=1.4,
...
drawer.rect(70, 444, 590, 140, rx=8, ry=8, fill="#FFFFFF", stroke=MINT_S,
...
for i, (nid, t) in enumerate([
    ...
    x = 96 + i * 176
    drawer.rect(x, 498, 156, 34, rx=5, ry=5, fill=MINT_F, stroke=MINT_S,
                stroke_width=1.1, node_id=nid, node_kind="op", role="node", bbox=True)
    drawer.text(x + 78, 515, t, 12, fill=INK, bbox=False)

drawer.rect(720, 444, 590, 140, rx=8, ry=8, fill="#FFFFFF", stroke=MINT_S,
...
drawer.rect(70, 858, 360, 98, rx=8, ry=8, fill=CORAL_F, stroke=CORAL_S,
...
drawer.rect(880, 858, 400, 98, rx=8, ry=8, fill=CORAL_F, stroke=CORAL_S,
            stroke_width=1.4, role="layer", bbox=False)
...
drawer.hexagon(560, 1218, 240, 34, fill=LILAC_F, stroke=LILAC_S, stroke_width=1.3,
